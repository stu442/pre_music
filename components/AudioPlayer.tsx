...
export default function AudioPlayer({ musicUrl, testId } : AudioPlayerProps) {
    ...
    return (
        <>
              <button className='flex justify-center items-center w-full h-full fixed left-[50%] top-[50%] translate-x-[-50%] translate-y-[-50%]' onClick={playPauseToggleMouse} onTouchEnd={e => playPauseToggleTouch(e)} data-cy={`audio-btn-${testId}`}>
              </button>
                {volume === 0 ? (
